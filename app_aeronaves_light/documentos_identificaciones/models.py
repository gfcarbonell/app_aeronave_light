...
class DocumentoIdentificacion(models.Model):
	nombre 	   						= models.CharField(
									  ...
									  unique=True, 
									  db_index=True, 
									  help_text='Escribir el nombre del documento de Identificación.')
	siglas							= models.CharField(
									  ...
									  unique=True, 
									  db_index=True,)
	numero_digito					= models.PositiveSmallIntegerField(
									  verbose_name = 'Número dígito(s)',
									  help_text="Ingresar el número de dígitos total del documento de indetificación.")
	slug							= models.SlugField(editable=False, max_length=255 ,unique=True, db_index=True, )
	
	#Métodos
	#Python 3.X
	def __str__(self):
		return self.get_siglas()
